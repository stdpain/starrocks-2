...
public class S3StorageHandler implements StorageHandler {
    ...
    public S3StorageHandler(CloudConfiguration cloudConfiguration) {
        ...
        AwsCredentialsProvider awsCredentialsProvider = awsCloudCredential
    ...
    public void getObject(String s3FilePath, String localPath) {
        try {
            ...
            LOG.error("connect to s3 failed with s3 exception", s3Exception);
            throw s3Exception;
        }  catch (Exception exception) {
            LOG.error("connect to s3 failed with exception", exception);
            throw exception;
        }
    }
