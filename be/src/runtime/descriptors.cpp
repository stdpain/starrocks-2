...
RowPositionDescriptor* RowPositionDescriptor::from_thrift(const TRowPositionDescriptor& t_desc, ObjectPool* pool) {
    ...
    case TRowPositionType::ICEBERG_V3_ROW_POSITION: {
        desc = pool->add(new RowPositionDescriptor(RowPositionDescriptor::ICEBERG_V3, t_desc.scan_table_id,
                                                   t_desc.row_source_slot, t_desc.fetch_ref_slots,
                                                   t_desc.lookup_ref_slots));
        break;
    }
    case TRowPositionType::OLAP_ROW_POSITION: {
        desc = pool->add(new RowPositionDescriptor(RowPositionDescriptor::OLAP_SCAN, t_desc.scan_table_id,
                                                   t_desc.row_source_slot, t_desc.fetch_ref_slots,
                                                   t_desc.lookup_ref_slots));
        break;
    }
