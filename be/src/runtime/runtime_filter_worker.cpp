...
void RuntimeFilterPort::publish_runtime_filters(const std::list<RuntimeFilterBuildDescriptor*>& rf_descs) {
    ...
    for (auto* rf_desc : rf_descs) {
        ...
        auto membership_filter = filter->get_membership_filter();
        if (rf_desc->join_mode() == TRuntimeFilterBuildJoinMode::BORADCAST && membership_filter->size() == 0) continue;
...
void RuntimeFilterMerger::merge_runtime_filter(PTransmitRuntimeFilterParams& params) {
    ...
    status->current_size += membership_filter->size();
    ...
    if (!status->can_use_bf) {
        ...
        if (rf_version >= RF_VERSION_V3) {
            for (auto& [_, rf] : status->filters) {
                rf = RuntimeFilterHelper::transmit_to_runtime_empty_filter(pool, rf);
            }
        } else {
            for (auto& [_, rf] : status->filters) {
