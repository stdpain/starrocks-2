...
public:
    ...
    virtual Status acquire_stream(std::shared_ptr<SpilledInputStream>* stream,
                                  std::queue<SpillRestoreTaskPtr>* tasks) = 0;

    virtual Status acquire_stream(const SpillPartitionInfo* partition, std::shared_ptr<SpilledInputStream>* stream,
                                  std::queue<SpillRestoreTaskPtr>* tasks) = 0;
...
public:
    ...
    SpilledFileGroup& file_group() { return _file_group; }

    Status acquire_stream(std::shared_ptr<SpilledInputStream>* stream, std::queue<SpillRestoreTaskPtr>* tasks) override;

    Status acquire_stream(const SpillPartitionInfo* partition, std::shared_ptr<SpilledInputStream>* stream,
...
public:
    ...
    Status acquire_stream(std::shared_ptr<SpilledInputStream>* stream,
                          std::queue<SpillRestoreTaskPtr>* tasks) override {
        return Status::OK();
    }
...
private:
    ...
    Status _spill_partition(SpillFormatContext& context, SpilledPartition* partition, ChunkProvider&& provider);

    Status _split_partition(SpillFormatContext& context, SpillerReader* reader, SpilledPartition* partition,
