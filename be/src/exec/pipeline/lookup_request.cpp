...
Status NativeLookUpTask::process(RuntimeState* state, const ChunkPtr& request_chunk) {
    ...
    for (auto* slot : tuple_desc->slots() | std::views::filter(should_keep)) {
...
auto NativeLookUpTask::_build_row_id_range(RuntimeState* state, const Columns& row_locator, SmallPermutation* output,
    ...
    *output = std::move(permutation);
    ...
    const auto& segment_id_column = ColumnHelper::get_data_column_by_type<TYPE_INT>(sorted_columns[1]);
    ...
    const auto segment_ids = segment_id_column->immutable_data();
    ...
    uint32_t cur_segment = segment_ids[0];
    ...
    for (size_t i = 1; i < num_rows; ++i) {
        ...
        uint32_t segment = segment_ids[i];
        rowid_t rowid = row_ids[i];

        bool same_locator = tablet == tablet_ids[i - 1] && segment == segment_ids[i - 1] && rowid == row_ids[i - 1];

        bool same_segment = tablet == cur_tablet && segment == cur_segment;
        ...
        if (!same_segment) {
            ...
            locators.emplace_back(cur_tablet, cur_segment, std::move(ranges));
