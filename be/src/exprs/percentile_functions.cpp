...
StatusOr<ColumnPtr> PercentileFunctions::lc_percentile_extract(FunctionContext* context, const Columns& columns) {
    auto return_type = context->get_return_type();
    auto logical_type = return_type.type;
    auto rate = ColumnHelper::get_const_value<TYPE_DOUBLE>(context->get_constant_column(1));
    ColumnPtr result = type_dispatch_column(logical_type, LCPercentileExtracter(), return_type, columns[0], rate);
    return result;
}
