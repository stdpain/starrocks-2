...
StatusOr<ColumnPtr> StringFunctions::split(FunctionContext* context, const starrocks::Columns& columns) {
    ...
    if (context->is_notnull_constant_column(0) && context->is_notnull_constant_column(1)) {
        ...
        array_binary_column->reserve(row_nums * split_string.size(), haystack_columns->immutable_bytes().size());
        ...
        if (delimiter.size == 0) { // split each character
            ...
            array_binary_column->reserve(haystack_columns->byte_size(), haystack_columns->immutable_bytes().size());
            ...
            array_binary_column->reserve(row_nums * 5, haystack_columns->immutable_bytes().size());
            for (int row = 0; row < row_nums; ++row) {
                array_offsets->append(offset);
                Slice haystack = string_viewer.value(row);
        ...
        array_binary_column->reserve(row_nums * 5, haystack_columns->immutable_bytes().size() * sizeof(uint8_t));
