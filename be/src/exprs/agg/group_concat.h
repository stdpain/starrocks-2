...
public:
    ...
    void update_batch_single_state(FunctionContext* ctx, size_t chunk_size, const Column** columns,
        ...
        if (ctx->get_num_args() > 1) {
            ...
            if (!ctx->is_notnull_constant_column(1)) {
                const auto* column_sep = down_cast<const InputColumnType*>(columns[1]);
                this->data(state).intermediate_string.reserve(column_val->immutable_bytes().size() +
                                                              column_sep->immutable_bytes().size());
            } else {
                auto const_column_sep = ctx->get_constant_column(1);
                Slice sep = ColumnHelper::get_const_value<TYPE_VARCHAR>(const_column_sep);
                this->data(state).intermediate_string.reserve(column_val->immutable_bytes().size() +
            ...
            const auto* column_val = down_cast<const InputColumnType*>(columns[0]);
            this->data(state).intermediate_string.reserve(column_val->immutable_bytes().size() + 2 * chunk_size);
        }
        ...
        if (src.size() > 1) {
            ...
            if (!src[1]->is_constant()) {
                ...
                if (chunk_size > 0) {
                    size_t old_size = bytes.size();
                    CHECK_EQ(old_size, 0);
                    size_t new_size = 2 * chunk_size * sizeof(uint32_t) + column_value->immutable_bytes().size() +
                                      column_sep->immutable_bytes().size();
                    bytes.resize(new_size);
