...
Status ArrowFlightWithRW::init(const std::string& uri_string, const PyFunctionDescriptor& func_desc) {
    ...
    Location location;
    FlightDescriptor descriptor = FlightDescriptor::Command(func_desc.to_json_string());
...
public:
    PyWorkerHandle(std::unique_ptr<ArrowFlightWithRW> arrow_client, std::unique_ptr<ChildProcess> process)
...
private:
    ...
    std::unique_ptr<ChildProcess> _process;
};
...
Status PyWorkerManager::_fork_py_worker(std::unique_ptr<ChildProcess>* child_process, std::string* unix_socket) {
    ...
    if (cpid == -1) {
        ...
        if (execvp(python_path.c_str(), args)) {
...
StatusOr<std::shared_ptr<arrow::RecordBatch>> ArrowFlightFuncCallStub::do_evaluate(RecordBatch&& batch) {
    return _py_worker_handle->client()->rpc(batch);
}

struct type_to_json {
    void operator()(const TypeDescriptor& type, rapidjson::Value& json, rapidjson::Document::AllocatorType& allocator) {
        json.SetObject();
        json.AddMember("type", type.type, allocator);
        json.AddMember("precision", type.precision, allocator);
        json.AddMember("scale", type.scale, allocator);
    }
};

std::string PyFunctionDescriptor::to_json_string() const {
    ...
    // Adding complex object for return_type
    rapidjson::Value returnTypeJson(rapidjson::kObjectType);
    type_to_json()(return_type, returnTypeJson, allocator);
    doc.AddMember("return_type", returnTypeJson, allocator);

    // Adding array of TypeDescriptors for input_types
    rapidjson::Value inputTypesJson(rapidjson::kArrayType);
    for (const auto& input_type : input_types) {
        rapidjson::Value inputTypeJson(rapidjson::kObjectType);
        type_to_json()(input_type, inputTypeJson, allocator);
        inputTypesJson.PushBack(inputTypeJson, allocator);
    }
    doc.AddMember("input_types", inputTypesJson, allocator);
