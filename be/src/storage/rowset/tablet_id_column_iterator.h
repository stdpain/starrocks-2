...
// Virtual column iterator that returns tablet ID for each row.
// This is used to expose tablet metadata as a queryable column without storing it.
class TabletIdColumnIterator final : public ColumnIterator {
    using ordinal_t = starrocks::ordinal_t;
    using rowid_t = starrocks::rowid_t;

public:
    explicit TabletIdColumnIterator(int64_t tablet_id) : _tablet_id(tablet_id) {}

    ~TabletIdColumnIterator() override = default;
    ...
    Status next_batch(size_t* n, Column* dst) override {
        ...
        // Fill with constant tablet_id value
    ...
    Status fetch_values_by_rowid(const rowid_t* rowids, size_t size, Column* values) override {
        return Status::NotSupported("Not supported by TabletIdColumnIterator: fetch_values_by_rowid");
    }
    ...
    Status next_dict_codes(size_t* n, Column* dst) override {
        return Status::NotSupported("Not supported by TabletIdColumnIterator: next_dict_codes");
    }

    Status decode_dict_codes(const int32_t* codes, size_t size, Column* words) override {
        return Status::NotSupported("Not supported by TabletIdColumnIterator: decode_dict_codes");
    }

    std::string name() const override { return "TabletIdColumnIterator"; }
