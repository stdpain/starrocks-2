    ...
    void build_rss_rowid_to_update_rowid() {
        ...
        for (uint32_t upt_row_id = 0; upt_row_id < src_rss_rowids.size(); upt_row_id++) {
            ...
            if (each_rss_rowid < UINT64_MAX) {
                auto rssid = (uint32_t)(each_rss_rowid >> 32);
                auto rowid = (uint32_t)(each_rss_rowid & ROWID_MASK);
...
private:
    ...
    Status _update_primary_index(const TabletSchemaCSPtr& tablet_schema, Tablet* tablet,
                                 ...
                                 PersistentIndexMetaPB& index_meta, vector<std::pair<uint32_t, DelVectorPtr>>& delvecs,
                                 PrimaryIndex& index);
